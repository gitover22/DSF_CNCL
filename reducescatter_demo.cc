...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < num_comms; i++) {
    ...
    CNRT_CHECK_TMP(cnrtMalloc(&send_buffer[i], buf_size * num_comms));
    CNRT_CHECK_TMP(cnrtMalloc(&recv_buffer[i], buf_size));
    CNRT_CHECK_TMP(cnrtMemset(send_buffer[i], 1, buf_size * num_comms));
  ...
  for (int i = 0; i < num_comms; i++) {
    std::thread t([=]() {
      CNCL_CHECK(cnclReduceScatter(send_buffer[i],
                                   recv_buffer[i],
                                   buf_count,
                                   cnclFloat32,
                                   cnclSum,
