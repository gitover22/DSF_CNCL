  ...
  for (int i = 0; i < num_comms; i++) {
    ...
    if (i == sender) {
      for (int j = 0; j < num_comms; j++) {
        CNRT_CHECK_TMP(cnrtMalloc(&send_buffer[j], buf_size));
        CNRT_CHECK_TMP(cnrtMemset(send_buffer[j], 1, buf_size));
      }
    }
  }
  ...
  for (int i = 0; i < num_comms; i++) {
    threads.push_back(std::thread([=]() {
      if (i == sender) {
        for (int j = 0; j < num_comms; j++) {
          CNCL_CHECK(cnclSend(
              send_buffer[j], buf_count, cnclFloat32, j, comms[i], queues[i]));
        }
      }
      CNCL_CHECK(cnclRecv(
          recv_buffer[i], buf_count, cnclFloat32, sender, comms[i], queues[i]));
    }));
  }
