...
int main(int argc, char* argv[]) {
  ...
  void** buffers = new void*[num_comms];
  ...
  int root = 0;
  int buf_count = (1 << 20);
  int buf_size = buf_count * sizeof(char);
  for (int i = 0; i < num_comms; i++) {
    CNRT_CHECK_TMP(cnrtSetDevice(dev_list[i]));
    CNRT_CHECK_TMP(cnrtQueueCreate(&queues[i]));
    CNRT_CHECK_TMP(cnrtMalloc(&buffers[i], buf_size));
    CNRT_CHECK_TMP(cnrtMemset(buffers[i], 0, buf_size));
  }
  CNRT_CHECK_TMP(cnrtMemset(buffers[root], 1, buf_size));
  ...
  for (int i = 0; i < num_comms; i++) {
    std::thread t([=]() {
      CNCL_CHECK(
          cnclBcast(buffers[i], buf_count, cnclChar, root, comms[i], queues[i]));
    });
  ...
  for (int i = 0; i < num_comms; i++) {
    CNRT_CHECK_TMP(cnrtFree(buffers[i]));
