...
int main(int argc, char* argv[]) {
  int num_comms = 8; // 通信子个数
  int* dev_list = new int[num_comms]; // 每个通信子对应的设备列表
  ...
  // 等待异步任务完成
  for (int i = 0; i < num_comms; i++) {
    CNRT_CHECK_TMP(cnrtQueueSync(queues[i])); // 阻塞当前线程，只到mlu设备上的队列任务完成
  }

  // finalize CNCL
  CNCL_CHECK(cnclDestroyComms(comms, num_comms)); // 在所有通信任务完成后释放通信子资源
